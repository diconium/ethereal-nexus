...
const redisConfig = {
  ...
  localCacheTTL: process.env.IN_MEMORY_CACHE_TTL
    ? parseInt(process.env.IN_MEMORY_CACHE_TTL)
    : 3600,
  redisCacheTTL: process.env.REDIS_CACHE_TTL
    ? parseInt(process.env.REDIS_CACHE_TTL) * 24 * 60 * 60
    : 30 * 24 * 60 * 60,
  keyPrefix:
  ...
  constructor() {
    ...
    this.redisClient.on('close', () => {
      logger.warn('Redis client connection closed', {
        operation: 'redis-close',
      });
    });

    this.subscriberClient.on('connect', () => {
      logger.info('Redis subscriber connected', {
        operation: 'redis-subscriber-connect',
      });
    });

    this.subscriberClient.on('error', (err) => {
      logger.error('Redis subscriber error', err, {
        operation: 'redis-subscriber-error',
      });
    });
    ...
    return tracer.startActiveSpan(
      ...
      async (span) => {
        try {
          ...
          span.setAttribute('cache.hit', true);
          span.setAttribute('cache.level', 'redis');
          ...
          logger.error('Failed to get cache entry from Redis', err as Error, {
            ...
            key: this.buildStructuredKey(key, tables),
            tables: tables?.join(','),
            isTag,
            isAutoInvalidate,
          });
    ...
    return tracer.startActiveSpan(
      ...
      async (span) => {
        try {
            ...
            try {
              ...
              if (keys.length > 0) {
                try {
                  ...
                  logger.error(
                    'Failed to publish cache invalidation message',
                    err as Error,
                    {
              ...
              logger.error(
                'Failed to invalidate cache for table',
                err as Error,
                {
          ...
          span.setAttribute('cache.invalidated_count', totalKeysInvalidated);
