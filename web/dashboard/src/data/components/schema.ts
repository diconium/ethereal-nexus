...
export const componentVersions = pgTable(
  ...
  (table) => {
    return {
      pk: primaryKey({
        columns: [table.component_id, table.version],
      }),
    };
  },
);
...
export const componentAssets = pgTable(
  'component_assets',
  {
    id: uuid('id').unique().notNull().defaultRandom(),
    component_id: uuid('component_id')
  ...
  (table) => {
    return {
      pk: primaryKey({
        columns: [table.component_id, table.version_id, table.url],
      }),
      version_id: index('version_id_idx').on(table.version_id),
    };
  },
);
export const componentAssetsRelations = relations(
  componentAssets,
  ({ one }) => ({
