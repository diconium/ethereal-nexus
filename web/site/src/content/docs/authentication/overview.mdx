...
Instead of using an API key, you can create a Bearer token using OAuth (e.g., from Keycloak) to establish a secure connection between Ethereal Nexus and the connector. This method provides an additional authentication option that integrates with identity providers for enhanced security.

 > Note: Using OAuth requires an extra connector package to be installed.
