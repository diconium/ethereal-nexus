...
export function ApiKeyForm({
  ...
  const defaultValues = apyKey ?? {
    ...
    permissions: {
      ...(session?.permissions as ApiPermissions),
      components: 'read',
    },
  };

  const form = useForm<NewApiKey>({
    resolver: zodResolver(newApiKeySchema as any),
    defaultValues,
  });
  ...
  const handleSubmit = async (data) => {
    ...
    if (key.success) {
      setKey(key.data.key);
      toast(`Api Key ${apyKey?.id ? 'updated' : 'added'} successfully!`);
    } else {
      toast(`Failed to ${apyKey?.id ? 'update' : 'add'} API key!`, {
  ...
  const noAccessPermissions =
    (apiKeyPermissions &&
      Object.values(apiKeyPermissions).every((value) => value === 'none')) ||
    undefined;
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-8">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                This is the key public display name.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => {
            return (
              ...
              >
                <FormLabel className="font-normal">Components</FormLabel>
                <FormControl>
                  <Select
        ...
        <ScrollArea className="h-[200px] gap-y-3">
          {availableProjects.map((item) => (
            <FormField
              ...
              name={'permissions'}
              render={({ field }) => {
                const restricted = isPermissionsHigher(
                  field.value?.[item.id],
                  session?.permissions[item.id],
                );
                return (
                  <FormItem
                    key={item.id}
                          ...
                          <SelectContent>
                            <SelectGroup>
                              ...
                              <SelectItem
                                value="read"
                                disabled={
                                  session?.permissions[item.id] === 'none'
                                }
                              >
                                Can read
                              </SelectItem>
        ...
        <DialogFooter className="flex items-center border-t p-4 sm:justify-between">
          <Button disabled={!!apiKey || noAccessPermissions} type="submit">
            Continue
          </Button>
        </DialogFooter>
      </form>
