...
export const newServiceUserSchema = newUserSchema
  .pick({
    id: true,
    name: true,
    issuer: true,
...
export const updatePasswordSchema = userSchema.pick({ id: true }).extend({
  password: newCredentialsUserSchema.shape.password,
  oldPassword: newCredentialsUserSchema.shape.password,
});
export type UpdatePassword = z.infer<typeof updatePasswordSchema>;

export const updateRoleSchema = userSchema.pick({ id: true, role: true });
...
export const userEmailSchema = newUserSchema
  .pick({
    email: true,
  })
  .extend({
...
export const newApiKeySchema = createInsertSchema(apiKeys).extend({
  permissions: apiPermissionsSchema,
});
