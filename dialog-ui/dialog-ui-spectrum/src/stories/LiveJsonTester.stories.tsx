...
const LiveJsonDialogTester: React.FC<{ storyChatbotApiUrl?: string }> = ({
  storyChatbotApiUrl,
}) => {
  const [dialogConfigJson, setDialogConfigJson] = useState(
    JSON.stringify(DEFAULT_DIALOG_CONFIG, null, 2),
  );
  const [initialValuesJson, setInitialValuesJson] = useState(
    JSON.stringify(DEFAULT_INITIAL_VALUES, null, 2),
  );
                  ...
                  children: [
                    {
                      ...
                      tooltip:
                        'Small text that appears above the main headline',
                    },
                    {
                      type: 'select',
                      id: 'tag',
  ...
  return (
    <View padding="size-300" maxWidth="100%">
      <Heading level={2} marginBottom="size-300">
        Live JSON Dialog Tester
      </Heading>

      <Text marginBottom="size-200">
        Test dialogs with real-time value updates. You can modify values while
        the dialog is open!
      </Text>
      ...
      >
        ...
        {isDialogOpen && (
          <Switch
            isSelected={showCurrentValues}
            onChange={setShowCurrentValues}
          >
            Show Current Values
          </Switch>
        )}
      ...
      {parseError && (
        ...
        >
          <Heading
            level={4}
            UNSAFE_style={{ color: 'red', margin: 0, marginBottom: '8px' }}
          >
            Error
          </Heading>
          <Text UNSAFE_style={{ color: 'red' }}>{parseError}</Text>
        </View>
      )}
        ...
        {isDialogOpen && showLiveEditor && (
          ...
          >
            ...
            <EnhancedDialogBody
              ...
              chatbotApiUrl={storyChatbotApiUrl}
            />
          </View>
        )}
      </Flex>
      ...
      {isDialogOpen && !showLiveEditor && (
        <DialogTrigger isOpen={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <Dialog
            size={'L'}
            UNSAFE_style={{ maxWidth: '1200px', width: '100%' }}
          >
            <Header>
              <Flex
                alignItems="center"
      ...
      <View marginTop="size-400">
        ...
        >
          <Text>
            <strong>1. Configure:</strong> Edit the Dialog JSON and Values JSON
            above.
          </Text>
          <br />
          <Text>
            <strong>2. Test:</strong> Click "Test Dialog" to open the dialog
            with your configuration.
          </Text>
          <br />
          <Text>
            <strong>3. Live Updates:</strong> Toggle "Show Live Editor" to see
            the dialog inline and edit values in real-time.
          </Text>
          <br />
          <Text>
            <strong>4. Update Values:</strong> Modify the Values JSON and click
            "Update Values Live" to see changes immediately.
          </Text>
        </View>
        ...
        >
          ...
          <Text>
            • Real-time value updates while dialog is open
            <br />
            • Live preview mode to see dialog inline
            <br />
...
const meta: Meta<typeof LiveJsonDialogTester> = {
  ...
  parameters: {
    ...
    docs: {
      description: {
        story:
          'Enhanced interactive dialog tester with real-time value updates. You can modify values while the dialog is open and see changes immediately.',
      },
    },
  },
  tags: ['autodocs'],
...
export const LiveTester_NoChatbot: Story = {
  name: 'Live JSON Dialog Tester (No Chatbot)',
  render: (args) => (
    <LiveJsonDialogTester storyChatbotApiUrl={args.storyChatbotApiUrl} />
  ),
  args: {
