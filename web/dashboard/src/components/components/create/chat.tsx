...
export default function Chat({ chatId }: ChatProps) {
  ...
  const [isPublishingComponent, setIsPublishingComponent] =
    useState<boolean>(false);
  const [isPreviewLoading, setIsPreviewLoading] = useState<boolean>(false);
  const [output, setOutput] = useState<StatusOutputType | undefined>();
  const [isWebContainerBooted, setIsWebContainerBooted] =
    useState<boolean>(false);

  const [updateComponentModalMetadata, setUpdateComponentModalMetadata] =
    useState<{ messageId: string; componentName: string } | undefined>();
  ...
  const {
    ...
    setIsComponentDetailsContainerOpen,
  ...
  useEffect(() => {
    ...
    bootWebContainer().then(() =>
      setOutput({ status: 'Success', message: 'WebContainer booted!' }),
    );
  ...
  useEffect(() => {
    const updateComponent = async () => {
      ...
      if (!serverUrlRef.current) {
        //happens when the user goes to another page and returns
        try {
          await setupInitialFiles();
        } catch (error) {
          setOutput({
      ...
      try {
        ...
        const propsToIndexFile = JSON.stringify(
          currentMessage.etherealNexusComponentMockedProps,
        );
        const indexFileUpdatedTemplate = createIndexFileTemplate(
          currentMessage?.componentName,
      ...
      lastReceivedMessage.toolInvocations?.map((toolInvocation) => {
        ...
        setCurrentMessage({
          ...
          etherealNexusComponentMockedProps:
            args.etherealNexusComponentMockedProps,
  ...
  const handleOnComponentCardClick = (
    ...
    setCurrentMessage({
      ...
      etherealNexusComponentMockedProps:
        result.etherealNexusComponentMockedProps,
    });
    setIsComponentDetailsContainerOpen(true);
  };
  ...
  const updateComponentMetadataAndPublishUpdatedComponent = async (
    messageId: string,
    name: string,
    version: string,
  ) => {
    function replaceEtherealNexusFileCodeWithUpdatedValues(code: string) {
      const updatedCode = code
        ?.replace(
          new RegExp(
            `const ${currentMessage?.componentName}: React\\.FC<Props>`,
            'g',
          ),
          `const ${name}: React.FC<Props>`,
        )
        .replace(
          new RegExp(`export default ${currentMessage?.componentName};`, 'g'),
          `export default ${name};`,
        )
        .replace(
          new RegExp(`export { ${currentMessage?.componentName} };`, 'g'),
          `export { ${name} };`,
        )
        .replace(
          new RegExp(
            `const schema = component\\({ version: '\\d+\\.\\d+\\.\\d+' }, dialogSchema\\);`,
            'g',
          ),
    ...
    if (messageId === currentMessage?.id) {
      setCurrentMessage({
        ...
        etherealNexusComponentMockedProps:
          currentMessage.etherealNexusComponentMockedProps,
        generatedCode: replaceEtherealNexusFileCodeWithUpdatedValues(
          currentMessage.generatedCode,
        ),
        fileName: `${name}.tsx`,
      });
      ...
      if (message.id === messageId) {
        return {
          ...
          toolInvocations: message.toolInvocations?.map((toolInvocation) => {
            updatedCode = replaceEtherealNexusFileCodeWithUpdatedValues(
              toolInvocation.args.etherealNexusFileCode,
            );

            return {
              ...toolInvocation,
              result: {
                // use ... to facilitate when new properties are added to the result
                description: toolInvocation.args.description,
                componentName: name,
                etherealNexusComponentMockedProps:
                  toolInvocation.args.etherealNexusComponentMockedProps,
                etherealNexusFileCode:
                  replaceEtherealNexusFileCodeWithUpdatedValues(
                    toolInvocation.args.etherealNexusFileCode,
                  ),
                fileName: `${name}.tsx`,
                updated: true,
  ...
  const executeViteReactPlugin = async (
    messageId: string,
    ...
    try {
      setOutput({
        status: 'Executing',
        message: 'Executing Ethereal Nexus plugin...',
      });
      ...
      if (exitCode === 0) {
        ...
        const upsertResponse = await upsertNewComponent(
          formData,
          componentName,
        );
        if (!upsertResponse.success) {
          if (
            upsertResponse.error.message === 'Component version already exists.'
          ) {
          ...
          setOutput({
            status: 'Error',
            message: 'There was an error publishing the component',
          });
        } else {
          setOutput({
            status: 'Success',
            message: 'Component was published successfully!',
          });
          toast(`${componentName} was published successfully!`);
        }
        setIsPublishingComponent(false);
      } else {
        setOutput({
          status: 'Error',
          message: 'Ethereal Nexus execution failed',
        });
        setIsPublishingComponent(false);
      }
    } catch (error) {
  ...
  return (
    <div className="flex h-full bg-gray-100">
      <div
        className={`flex flex-col ${isComponentDetailsContainerOpen ? 'w-1/2' : 'w-full'} transition-all duration-300 ease-in-out`}
      >
        ...
        <div className="p-4 bg-white border-t">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <TextArea
              ...
              className="flex-1 px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus-visible:ring-accent focus-visible:border-transparent"
            />
            <div className="flex">
              {!isLoadingNewMessage ? (
                <Button
                  ...
                  type="submit"
                  className="px-4 py-2 text-white bg-accent rounded-md hover:bg-accent/90 focus:outline-hidden focus:ring-2 focus:ring-accent focus:ring-offset-2 h-full"
                >
      ...
      {isComponentDetailsContainerOpen && (
        <div className="w-1/2 border-l relative flex flex-col">
          <div className="flex flex-1 flex-col overflow-auto relative">
            <Button
              variant="ghost"
              className="absolute top-2 right-2 z-10 p-1 h-8 w-8 rounded-full bg-white shadow-md hover:bg-gray-100 focus:outline-hidden focus:ring-2 focus:ring-accent"
              onClick={closeComponentDetailsContainer}
            >
              <X className="h-4 w-4" />
            </Button>
            <GeneratedCodeDetailsContainer
      ...
      {updateComponentModalMetadata && (
        <UpdateComponentMetadataModal
          ...
          updateComponentMetadata={
            updateComponentMetadataAndPublishUpdatedComponent
          }
        />
      )}
    </div>
