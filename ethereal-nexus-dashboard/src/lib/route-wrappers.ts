...
>(cb: WrapperCallback<Req, Ext, Res>) {
  ...
  >(handler: (req: HReq & Req, ext?: HExt) => HRes | Promise<HRes>) {
    // the new handler
...
export const authenticatedWithKey = wrapper(
  ...
  HExt = DefaultExt & AuthenticatedWithApiKeyUser,
  HReq = Request,
  HRes = Response | Promise<Response>,
>(
  handler: (req: HReq & NextRequest, ext?: HExt) => Promise<HRes> | HRes,
) => (req: HReq & NextRequest, ext?: HExt) => Promise<HRes> | HRes;
