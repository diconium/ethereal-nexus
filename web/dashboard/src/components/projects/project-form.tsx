...
export default function ProjectsForm({
  ...
  const hasWritePermissions =
    ...
    (project?.id &&
      session?.permissions &&
      ['write', 'manage'].includes(session?.permissions[project.id] || ''));
  ...
  const onSubmit = async (data: ProjectInput) => {
    ...
    browserLog.info(
      `Project form submission started: ${isUpdate ? 'updating' : 'creating'} project`,
      {
    ...
    try {
      ...
      if (!projects.success) {
        ...
        toast(
          `Failed to ${data.id ? 'update' : 'create'} project "${data.name}"!`,
        );
        return;
      }
      ...
      toast(`Project ${project?.id ? 'update' : 'create'}d successfully!`);
      ...
      browserLog.error(
        `Unexpected error during project ${isUpdate ? 'update' : 'creation'}`,
        error as Error,
        {
          operation: 'project-form-submit',
          action: isUpdate ? 'update' : 'create',
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex items-center gap-14">
          ...
          <Button
            ...
            type="submit"
            size="default"
          >{`${project?.id ? 'Save' : 'Create project'}`}</Button>
        </div>
      </form>
    </Form>
  );
