...
export function UpdateComponentMetadataModal({
  metadata,
  onClose,
  updateComponentMetadata,
}: MetadataModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    toast(`Component metadata updated successfully`, {
    ...
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Update the component name to create a new one or update the version
            to add a new version of the {metadata.componentName} component
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit}>
          ...
          <div className="mt-6 flex justify-end gap-3">
            <Button
              onClick={onClose}
              variant="ghost"
              className="px-4 py-2 text-accent font-bold text-base p-0"
            >
              Cancel
            </Button>
            <Button type="submit" size="default" className="px-4 py-2">
